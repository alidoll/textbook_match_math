...
def _cache_dir() -> Path:
    ...
    return app_root() / "data" / "base_data" / "cache" / "page_text"

_CACHE_DIR = None  # 懒加载，避免模块导入时 __file__ 路径问题

# 提示词变更时递增，避免沿用旧缓存（含：夹注旁汉字禁漏；数学分式数字校对）
PAGE_TEXT_PROMPT_VERSION = 33
# 附录字表/词表专用 OCR 提示词版本（与普通页缓存隔离）
PAGE_TEXT_APPENDIX_PROMPT_VERSION = 3
...
17. 【数学公式·LaTeX】整式/分式/根号等印刷公式用 LaTeX，行内用 `$...$`：
   - 分式写 `$\\frac{分子}{分母}$`（JSON 中反斜杠必须写成 `\\\\`，即输出字面 `\\frac`）；
   - 正确：`$\\frac{m}{n+20}$`、`$x^{2}$`；错误：把反斜杠写成别的符号，或漏掉分式结构写成 `m/n`（除非页上本来就是斜杠写法）。
   - 同一叙述句与紧随其后的一排公式可同框；公式与下一段说明勿强行拆碎。
   - 【数字 vs 字母·最高优先级】分式/整式里的数字必须按页上字形抄，严禁与形近字母互替：
     · 数字 **5** ≠ 字母 **s / S**（错误案例：页上 `$\\frac{1}{5}$` 写成 `$\\frac{1}{s}$`）；
     · 数字 **0** ≠ 字母 **o / O**；数字 **1** ≠ **l / I**；数字 **2** ≠ **z / Z**；
     · 多位数须抄全每一位：页上 `n+20` 禁止写成 `n+2`（漏掉末位 0）；页上 `10` 禁止写成 `1` 或 `lo`。
   - 斜体变量（a/b/c/m/n）与正体数字（1/5/20）并存时，先判断「是否为阿拉伯数字」再输出。"""

# 附录专用 system：独立提示，不继承正文页的「合框」规则，避免模型整表一大框
PAGE_TEXT_APPENDIX_SYSTEM = """你是 K12 语文教材【附录字表/词表】逐行分区专家。根据附带页图，按「印刷横行」划出文字区域。

【唯一分框粒度·强制】
1. 表内每一个带行号的印刷横行 = **恰好一个** text 框（扁横条）。页上有几行编号行，就输出几个 text 框。
2. **禁止**把多行或整张表收进同一个大框。错误：从第 1 行罩到第 11 行的一个大红框。
3. 正确示例：行「1 …」一框、行「2 …」一框、行「3 …」一框、行「4 …」一框……各自独立；bbox 只盖住该行的行号+该行汉字及其上方拼音。
4. 同一行内多个字及其上方拼音同属这一行的同一个框；**不要**把一行拆成逐字小框。
5. 页顶标题「识字表」「写字表」「词语表」（可含 ①）单独 **title** 框。
6. 页侧「语文园地」等小栏目名单独 title/text 框；页底脚注说明单独 text 框。二者都不要并进字表行。

【text 写法】
- 编号行：行号写在开头，后接本行全部汉字；有夹注拼音时写成「字(pīn)」，仅页上印了音的字才加括号。
- 多行内容禁止用换行塞进同一个 text；一行一个区域。
""" + _PINYIN_FIVE_TONES_GUIDE + """
【bbox】
- 0~1 相对整页；每个字表行的框应较扁（高度大约只盖一行拼音+汉字），上下相邻行的框不要重叠成整页柱状。
- bbox 须盖住该行拼音顶到汉字底。

【不要框选】
水印（如「严禁外传」「CHHT」）、页码、纯装饰。

【role】
title / text（字表行用 text）。
"""


def page_text_prompt_version(*, appendix: bool = False, subject: str | None = None) -> int | str:
    """缓存用 prompt_version；附录 / 数学科目与普通模式隔离。"""
    if appendix:
        return f"{PAGE_TEXT_PROMPT_VERSION}a{PAGE_TEXT_APPENDIX_PROMPT_VERSION}"
    sub = (subject or "").strip()
    if sub == "数学":
        return f"{PAGE_TEXT_PROMPT_VERSION}m"
    return PAGE_TEXT_PROMPT_VERSION
...
class LlmMathFormulaAudit(BaseModel):
    """数学公式校对：与输入子集等长的校对后 text。"""

    corrected_texts: list[str] = Field(default_factory=list)


PAGE_TEXT_MATH_AUDIT_SYSTEM = """你是 K12 数学教材「公式字形」校对员。对照页图，只修正各段 OCR text 里的数学公式字符错误。

硬性规则：
1. 【只改公式字符】对照页图核验 `$...$` / `$$...$$` 内的每个符号；汉字叙述、标点、换行、段落结构一律保持原样。
2. 【数字 vs 字母】按页上印刷字形区分，严禁互替：
   - 数字 5 ≠ 字母 s/S（典型错：`$\\frac{1}{5}$` 写成 `$\\frac{1}{s}$`，`$\\frac{a}{5}$` 写成 `$\\frac{a}{s}$`）
   - 数字 0 ≠ o/O；数字 1 ≠ l/I；数字 2 ≠ z/Z
3. 【多位数抄全】多项式/常数每一位都要在：页上 `n+20` 禁止写成 `n+2`；页上 `10`/`100` 禁止截断。
4. 【LaTeX 结构】保持 `$\\frac{分子}{分母}$` 等结构；JSON 里反斜杠写成 `\\\\`；不要把竖直分式改成斜杠 `a/b`，除非页上本来就是斜杠。
5. 【禁止臆造】页上没有的字母/数字不要补；不要改写题意。
6. 输出 corrected_texts，长度必须与输入段数相同；无改动的段原样返回。"""
...
def page_text_math_audit_enabled() -> bool:
    """数学公式二次校对；默认开启。设 PAGE_TEXT_MATH_AUDIT=0 可关。"""
    import os
    ...
    from ...repo_paths import app_root
    load_dotenv(app_root() / ".env", encoding="utf-8-sig", override=True)
    raw = os.getenv("PAGE_TEXT_MATH_AUDIT", "1").strip().lower()
    return raw not in ("0", "false", "no", "off")
...
_MATH_FORMULA_HINT_RE = re.compile(
    r"(\$\$[\s\S]+?\$\$|\$[^$\n]+?\$|\\frac\b|\frac\b|\x0crac\b|[↑⬆]\s*rac\b)"
)


def _region_needs_math_audit(text: str) -> bool:
    t = text or ""
    if not t.strip():
        return False
    return bool(_MATH_FORMULA_HINT_RE.search(t))


def _invoke_math_formula_audit_llm(
    image_url: str,
    *,
    page_index: int,
    regions: list[LlmTextRegion],
    ...
    """对照页图校对分式/整式里的数字与字母；失败则返回原文。"""
    if not regions or not page_text_math_audit_enabled():
        return regions
    # 数学册整页优先；其它学科仅校含公式的段
    is_math = (subject or "").strip() == "数学"
    indexed = [
        (i, r)
        for i, r in enumerate(regions)
        if _region_needs_math_audit(r.text or "")
        or (is_math and "$" in (r.text or ""))
    ]
    # 无公式特征则跳过（避免数学叙述页白跑一轮）
    indexed = [(i, r) for i, r in indexed if _region_needs_math_audit(r.text or "")]
    if not indexed:
        return regions

    from langchain_core.messages import HumanMessage, SystemMessage

    payload = [{"index": i, "text": (r.text or "")[:4000]} for i, r in indexed]
    llm = build_chat_openai(max_tokens=4096, model=llm_vision_model(), timeout=120.0)
    messages = [
        SystemMessage(content=PAGE_TEXT_MATH_AUDIT_SYSTEM),
        HumanMessage(
            content=[
                {
                    "type": "text",
                    "text": (
                        f"请校对【数学教材 P{int(page_index)}】下列 {len(payload)} 段 text 中的公式。"
                        "重点核对：数字 5 是否被误成 s、数字 0 是否漏写（如 n+20→n+2）、"
                        "以及其它数字/字母形近混淆。按输入顺序返回等长 corrected_texts。\n"
                        + json.dumps(payload, ensure_ascii=False)
                    ),
                },
                {"type": "image_url", "image_url": {"url": image_url}},
    ...
    try:
        structured = llm.with_structured_output(LlmMathFormulaAudit)
        audit = structured.invoke(messages)
        fixed_list = list(audit.corrected_texts or [])
        if len(fixed_list) != len(payload):
            logger.warning(
                "math formula audit subset length mismatch P%s: in=%s out=%s",
                page_index,
                len(payload),
                len(fixed_list),
            )
        ...
        for (idx, _), fixed in zip(indexed, fixed_list):
            text = (fixed or "").strip() or (merged[idx].text or "")
            text = repair_latex_json_control_chars(text)
            merged[idx] = merged[idx].model_copy(update={"text": text})
        return merged
    except Exception as exc:
        logger.warning(
            "math formula audit P%s failed, keep raw OCR: %s", page_index, exc
        )
        return regions
...
def _invoke_page_text_llm(
    ...
    *,
    ...
    llm = build_chat_openai(
        max_tokens=8192 if appendix else 4096,
        model=llm_vision_model(),
        timeout=180.0 if appendix else 120.0,
    )
    if appendix:
        system = PAGE_TEXT_APPENDIX_SYSTEM
        human_text = (
            f"请分析【附录字表/词表 P{int(page_index)}】，输出 role、text、bbox JSON。\n"
            "硬性要求：\n"
            "1) 每一个带行号的印刷横行各自一个 text 框（如 1 行、2 行、3 行、4 行……分别出框）；\n"
            "2) 禁止整表一个大框；每个框只要盖住该行行号+该行字及其上方拼音；\n"
            "3) 标题「识字表/写字表/词语表」单独 title；「语文园地」与脚注说明各自单独框；\n"
            "4) 夹注拼音仅页上有音才写 字(pīn)；调号只看页上形状；\n"
            "5) 不要框选水印、页码。"
        )
    else:
        system = PAGE_TEXT_SYSTEM
        if is_math:
            human_text = (
                f"请分析【数学教材 P{int(page_index)}】，输出所有文字区域的 role、text、bbox JSON。"
                "公式用 LaTeX：`$\\frac{分子}{分母}$`（JSON 反斜杠写成 `\\\\`）。"
                "【数字 vs 字母】务必按页上字形：数字 5 禁止写成 s；数字 0 禁止漏写或写成 o；"
                "多位数抄全（页上 n+20 禁止写成 n+2）。"
                "斜体字母 a/b/c/m/n 与正体数字 1/5/20 并存时先判是否为阿拉伯数字。"
                "叙述句与紧随的一排分式可同框；不要框选水印、页码。"
            )
        else:
            human_text = (
    ...
    if skip_post_audit:
        return LlmPageText(regions=regions)
    regions = apply_page_text_post_audits(
        image_url,
        regions,
...
def apply_page_text_post_audits(
    image_url: str,
    regions: list[Any],
    *,
    page_index: int,
    ...
    """文字主 OCR 之后的校对轮（公式/拼音）；可与图片 OCR 并行调用。"""
    if appendix:
        return list(regions or [])
    is_math = (subject or "").strip() == "数学"
    out = list(regions or [])
    if is_math:
        return _invoke_math_formula_audit_llm(
            image_url, page_index=page_index, regions=out, subject=subject
        )
    out = _invoke_pinyin_audit_llm(
        image_url, page_index=page_index, regions=out
    )
    if page_text_tone_verify_enabled():
        out = _invoke_pinyin_tone_verify_llm(
            image_url, page_index=page_index, regions=out
        )
    return _invoke_math_formula_audit_llm(
        image_url, page_index=page_index, regions=out, subject=subject
...
def finalize_deferred_page_text_audit(pending: dict[str, Any]) -> list[dict[str, Any]]:
    """对 defer 的文字 OCR 做校对并写缓存，返回原子列表。"""
    image_url = str(pending.get("image_url") or "")
    regions_raw = pending.get("regions") or []
    pi = int(pending.get("page_index") or 1)
    appendix = bool(pending.get("appendix"))
    subject = pending.get("subject")
    if not image_url or not regions_raw:
        return regions_to_raw_atoms(regions_raw, page_index=pi, appendix=appendix)
    regions = [
        LlmTextRegion.model_validate(r) if isinstance(r, dict) else r
        for r in regions_raw
    ]
    audited = apply_page_text_post_audits(
        image_url,
        regions,
        page_index=pi,
        subject=subject,
    ...
    region_dicts = [
        ...
        for r in audited
        if (getattr(r, "text", None) or (r.get("text") if isinstance(r, dict) else "") or "").strip()
    ]
    key = str(pending.get("cache_key") or "page")
    content_hash = pending.get("content_hash")
    prompt_ver = pending.get("prompt_ver") or page_text_prompt_version(
        appendix=appendix, subject=subject
    )
    if content_hash:
    ...
    return regions_to_raw_atoms(audited, page_index=pi, appendix=appendix)
...
def repair_latex_json_control_chars(text: str) -> str:
    """恢复被 JSON 控制转义吃掉的 LaTeX 反斜杠。

    模型常输出未转义的 ``\\frac``；``json.loads`` / structured 解析会把 ``\\f``
    变成换页符 (U+000C)，界面上就像 ``↑rac`` / ``⬆rac``，公式无法渲染。
    """
    if not text:
        return text or ""
    t = text
    # JSON \\f → form feed；\\b → backspace（\\begin 等）
    if "\x0c" in t:
        t = t.replace("\x0c", "\\f")
    if "\x08" in t:
        t = t.replace("\x08", "\\b")
    # 个别链路把换页符再显示/落成箭头
    t = re.sub(r"[↑⬆]\s*rac\b", r"\\frac", t)
    t = re.sub(r"[↑⬆]\s*begin\b", r"\\begin", t)
    t = re.sub(r"[↑⬆]\s*beta\b", r"\\beta", t)
    return t
...
def repair_common_pinyin_tone_ocr(text: str) -> str:
    ...
    - 顺带恢复被 JSON 吃掉的 LaTeX ``\\frac`` 等
    """
    import unicodedata

    if not text:
        return text or ""
    t = repair_latex_json_control_chars(text)
    t = normalize_pinyin_tone_marks(t)
    # 啊 + 全角括号 → 半角，便于统一匹配
    t = re.sub(
...
def regions_to_raw_atoms(
    ...
    *,
    ...
    # 附录页：不做合框/拆行后处理，完全保留模型逐行输出；普通页仍合并识字加油站对照表
    if not appendix:
...
def extract_page_text_regions_from_image(
    *,
    ...
    subject: str | None = None,
    skip_post_audit: bool = False,
    audit_pending: dict[str, Any] | None = None,
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """
    从页图 PNG 跑豆包文字 OCR（教材对比等无 lesson blob 场景）。
    与 extract_page_text_regions 共用模型与缓存目录，cache 键用 content_hash。
    appendix=True 时仅用于附录识字表/写字表/词语表：强制一行一原子。
    subject=「数学」时用数学公式强化提示 + 公式校对，并隔离缓存版本。
    skip_post_audit=True 时跳过公式/拼音校对轮，并把待校对上下文写入 audit_pending，
    供与图片 OCR 并行后再 finalize_deferred_page_text_audit。
    """
    pi = int(page_index)
    ...
    prompt_ver = page_text_prompt_version(appendix=appendix, subject=subject)
    if not force_refresh:
        cached = load_page_text_cache(key, pi)
        if (
            cached
            and cached.get("blob_id") == content_hash
            and cached.get("prompt_version") == prompt_ver
            and isinstance(cached.get("regions"), list)
            and cached.get("regions")
        ):
            atoms = regions_to_raw_atoms(
                cached["regions"], page_index=pi, appendix=appendix
            )
            if atoms:
                return atoms, {
                    "source": "cache",
    ...
    try:
        ...
        if skip_post_audit:
            ...
            atoms = regions_to_raw_atoms(result.regions, page_index=pi, appendix=appendix)
            return atoms, {
                "source": "doubao_preaudit",
                "region_count": len(atoms),
                "engine": "doubao",
                "model": llm_vision_model(),
                "appendix": appendix,
