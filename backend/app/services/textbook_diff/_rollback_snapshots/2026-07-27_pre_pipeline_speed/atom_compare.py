...
_TEXT_COMPARE_CACHE_VERSION = 50  # v50: 注释包裹符 〔〕≈[] 软忽略
...
def _doubao_text_atoms_for_page(
    *,
    ...
    try:
        ...
        return None
    atoms = _drop_watermark_atoms(atoms or [])
    if atoms:
        _log.info(
            "diff doubao text OCR %s p%d: %d atoms (%s)",
            label,
            page_1,
            len(atoms),
            meta.get("source"),
...
def _drop_watermark_atoms(atoms: list[dict]) -> list[dict]:
    from ..llm.page_text_extract import (
    ...
    return _merge_adjacent_form_pair_list_atoms(drop_orphan_pinyin_atoms(out))
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="diff_atom_") as td:
        ...
        if (ocr_phase or "").strip().lower() == "text":
            ...
            if doubao:
                return _drop_watermark_atoms(doubao)
...
def _build_atoms_payload(entry: dict) -> dict:
    # 展示前再跑一遍：旧缓存框未盖注音 / 残留孤立拼音碎框时也能立刻修正
    old_raw = _drop_watermark_atoms(_atoms_for_api(entry.get("old_raw") or []))
    new_raw = _drop_watermark_atoms(_atoms_for_api(entry.get("new_raw") or []))
    ...
    *,
    ...
    if not entry.get("old_raw") and not entry.get("new_raw"):
        if not entry.get("text_compare") and not entry.get("image_compare"):
            return None
    return _build_atoms_payload(entry)
...
def compare_page_text(
    ...
    *,
    ...
    if block_rows and not changed_blocks:
        ...
        r.get("change") in soft_ok or r.get("change") == "仅标点差异" or "标点" in (r.get("change") or "")
        for r in block_rows
    ):
        if any(r.get("change") == "仅标点差异" for r in changed_blocks):
            verdict = "仅标点差异"
...
def run_diff_page_ocr_both(
    *,
    ...
    """当前页对：旧侧 → 新侧依次跑同一阶段 OCR（与小学科学两阶段一致）。"""
    run_diff_page_ocr(
        old_vol=old_vol,
        new_vol=new_vol,
        old_page=old_page,
