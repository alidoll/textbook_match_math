...
def _pipeline_page_workers() -> int:
    """整册/课时页级并发数。环境变量 DIFF_PIPELINE_PAGE_WORKERS，默认 3，上限 8。"""
    raw = (os.getenv("DIFF_PIPELINE_PAGE_WORKERS") or "3").strip()
    try:
        n = int(raw)
    except ValueError:
        n = 3
...
def list_volume_page_steps(*, old_vol: Volume, new_vol: Volume) -> list[dict[str, Any]]:
    """整册：仅展开状态为「待对比」(suggested) 的课对页；已跑过/已确认的跳过。"""
    rows = (
    ...
    _log.info(
        "volume pipeline steps=%s (skip confirmed=%s pending_review=%s other=%s)",
        len(steps),
        skipped_confirmed,
        skipped_pending,
                        ...
                        with confirmed_lock:
                            if uid not in confirmed_uids:
                                try:
                                    ...
                                    # Test：对比内容只落本地 JSON，课对状态等「提交到数据库」再写
                                    if should_persist_compare_to_db(old_vol, new_vol):
...
def start_pipeline(
    *,
    ...
    if scope == "lesson":
        ...
        steps = list_volume_page_steps(old_vol=old_vol, new_vol=new_vol)
